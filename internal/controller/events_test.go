			...
			wantEvents: []capturedEvent{
				{
					...
					note: fmt.Sprintf(
						...
						shortDigest(testDigestB),
						shortDigest(testDigestA),
					),
				},
				{
					regarding: "pool-events",
...
func TestNodeEvent(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "first observed idle does not emit",
			status:          metav1.ConditionTrue,
			reason:          bootcv1alpha1.NodeReasonIdle,
			wantObservation: "True:Idle:" + testImageDigestRefB,
		},
		{
			name:            "idle after rollout emits",
			status:          metav1.ConditionTrue,
			reason:          bootcv1alpha1.NodeReasonIdle,
			previous:        "False:Rebooting:" + testImageDigestRefB,
			wantObservation: "True:Idle:" + testImageDigestRefB,
			wantReason:      bootcv1alpha1.NodeReasonIdle,
			wantNote:        "Node is up to date with image " + testImageDigestRefB,
		},
		{
			name:            "idle after idle does not re-emit",
			status:          metav1.ConditionTrue,
			reason:          bootcv1alpha1.NodeReasonIdle,
			previous:        "True:Idle:" + testImageDigestRefB,
			wantObservation: "True:Idle:" + testImageDigestRefB,
		},
		{
			name:            "staging",
		...
		},
		{
			...
			status:          metav1.ConditionFalse,
			reason:          "Unknown",
			wantObservation: "False:Unknown:" + testImageDigestRefB,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			observation, reason, note := nodeEvent(node, tt.previous)

			g.Expect(observation).To(Equal(tt.wantObservation))
			g.Expect(reason).To(Equal(tt.wantReason))
			if tt.wantNote == "" {
				g.Expect(note).To(BeNil())
			} else {
				g.Expect(note).NotTo(BeNil())
				g.Expect(note.Note()).To(Equal(tt.wantNote))
			}
		})
	}
}
...
func TestShortDigest(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "full digest is abbreviated",
			digest: testDigestA,
			want:   testDigestA[:len("sha256:")+12],
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			g.Expect(shortDigest(tt.digest)).To(Equal(tt.want))
		})
	}
}
...
func TestRolloutAndNodeEvents(t *testing.T) {
	...
	g.Eventually(func() ([]eventsv1.Event, error) {
		...
		HaveField("Note", "Rollout started toward digest "+shortDigest(testDigestB)),
	)))
	...
	g.Eventually(func() ([]eventsv1.Event, error) {
		...
		HaveField("Action", eventActionRollout),
		HaveField("Note", "Rollout completed at digest "+shortDigest(testDigestB)),
	)))
	...
	g.Eventually(func() ([]eventsv1.Event, error) {
		...
		HaveField("Action", eventActionNodeUpdate),
		HaveField("Note", "Node is up to date with image "+testImageDigestRefB),
		HaveField("Related", And(
			Not(BeNil()),
			HaveField("Name", poolName),
		)),
	...
	g.Consistently(func() (map[string]int, error) {
		...
		bootcv1alpha1.NodeReasonIdle:      1,
	}))
}
